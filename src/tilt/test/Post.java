...
public class Post extends Test
{
    ...
    +"/\";\n\t$.post( localurl, $(\"#main\").serialize(), function"
    +"( data ) {\n\t$(\"#left\").empty();\n\t$(\"#left\").html(dat"
    +"a);\n\tenableAll();\n\t},\"text\").fail(function(xhr, status"
    +", error){\n\talert(status);\n\tdisableAll();\n\t});\n});\n$("
    +"\"#original\").click(function(){\n\t$(\"#left\").empty();\n\t"
    +"$(\"#left\").html('<img width=\"500\" src=\"'+getUrlFromLoc"
    +"(\"original\")+'\">');\n});\n$(\"#greyscale\").click(functio"
    +"n(){\n\t$(\"#left\").empty();\n\t$(\"#left\").html('<img wid"
    ...
    +"'+getUrlFromLoc(\"cleaned\")+'\">');\n});\n$(\"#baselines\""
    +").click(function(){\n\tvar url = escapeUrl(getUrlFromLoc(\"b"
    +"aselines\"));\n\t$(\"#left\").empty();\n\t$(\"#left\").html("
    +"'<img width=\"500\" src=\"'+getUrlFromLoc(\"baselines\")+'\""
    +">');\n});\n$(\"#selections\").change(function(e){\n\tvar val"
    +" = $(\"#selections\").val();\n\t$(\"#geojson\").val(unescape"
    +"Quotes(val));\n\tdisableAll();\n});\n});\n";
